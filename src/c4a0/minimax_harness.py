...
class MinimaxHarnessConfig:
    ...
    def __post_init__(self) -> None:
        ...
        if not 1 <= self.max_depth <= 12:
            raise ValueError("max_depth must be between one and twelve")
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            if self.stopped_at is not None and len(self.levels) > 1
                ...
                self.levels[-1].opponent
                if self.stopped_at is None and self.levels
                else None
            ),
        }


_WINNING_MASKS = tuple(
    sum(1 << ((row + step * dr) * 7 + col + step * dc) for step in range(4))
    for row in range(6)
    for col in range(7)
...
def _has_four(bits: int) -> bool:
    return any(bits & mask == mask for mask in _WINNING_MASKS)
...
class _MinimaxEvaluator:
    ...
    def __init__(self, depth: int, should_cancel=None, deadline=float("inf")):
        self.depth = depth
    ...
    def _score(self, current: int, opponent: int, depth: int) -> int:
        ...
        if self.nodes % 256 == 0:
            if self.should_cancel is not None and self.should_cancel():
                raise TrainingCancelled("Minimax evaluation cancelled")
            if time.monotonic() >= self.deadline:
                raise TimeoutError("Minimax evaluation exceeded its deadline")
...
def evaluate_minimax_ladder(
    ...
    for depth in [None, *range(1, config.max_depth + 1)]:
        ...
        opponent = (
            None
            if depth is None
            else _MinimaxEvaluator(
                depth, should_cancel=should_cancel, deadline=deadline
            )
        )
        ...
        while games < config.games_per_level:
            pair_count = min(
                config.pair_batch_size, (config.games_per_level - games) // 2
            )
            requests = []
                    ...
                    [
                        ...
                        c4a0_cpp.GameRequest(
                            c4a0_cpp.GameMetadata(
                                next_game_id + 1, OPPONENT_ID, MODEL_ID
                            )
                        ),
                    ]
                )
            ...
            for game in result.results:
                ...
                model_score = (
                    red_score
                    if game.metadata.player0_id == MODEL_ID
                    else 1.0 - red_score
                )
                if model_score == 1.0:
                    wins += 1
